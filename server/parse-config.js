...
if (gaeId)
    parseConfig.filesAdapter = new GCSAdapter(
        ...
        envConfig.gcpProjectId + '.appspot.com', { directAccess: true }
    )
...
if (config.dev && config.dev.gcpProjectNumber && config.gcpServerKey)
    parseConfig.push.android = {
        senderId: config.dev.gcpProjectNumber,
        apiKey: config.gcpServerKey
    }
else
    console.log('Unable to configure dev push notification for Android')
