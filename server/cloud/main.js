...
Parse.Cloud.define("ProcessMatch", function(request, response) {
    ...
    return matchQuery.first(function(result) {
        ...
        console.log('fetch user table');
        // Send the push notification to the other user when Match
        var userName = '';
        Parse.Cloud.useMasterKey();
        var new_query = new Parse.Query("User")
        new_query.include("profile")

        new_query.get(userId, {
            success: function(userObj) {
                profileObj = userObj.get('profile');

                if (profileObj) {
                    userName = profileObj.get('name');
                    ...
                    Parse.Push.send({
                        ...
                        success: function() {
                            // response.success(match.toJSON())
                            console.log('Success sending push notification for matching a account. ')
                        },
                        error: function(error) {
                            console.log('Error sending push notification for matching a account. ' + JSON.stringify(error))
                        }
                    })
                }
            },
...
Parse.Cloud.afterSave('ChatMessage', function(request) {
    ...
    if (message.get('text'))
        alert = senderName + ' has messaged you'
