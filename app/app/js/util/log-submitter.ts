angular.module('ionicApp')
	 ...
	 */
	.config([ '$provide', 'parseServerUrl', function( $provide, parseServerUrl ) {

		if (!parseServerUrl) {
			console.error('ERROR: parseServerUrl config value not configured for logging')
			return
		}
			...
			$delegate.error = function() {
				...
				try {
					...
					if (errorMessage !== lastError && Date.now() > (lastSubmitTime + MIN_TIME_DELAY)) {
						...
						xmlHttp.open('POST', parseServerUrl + '/client-log', true)
